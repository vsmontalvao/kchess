...
enum piece mkCmpMove(SearchBoard *board, Player turn, move m)
{
	...
	else if(turn && board->board[initial(m)]==wpawn && initial(m)%8 == 6)
	{
		...
		// Ganho de material:
		board->mat[whites] += weights[*(reinterpret_cast<piece*>(&a))] - weights[wpawn];
	}
	...
	else if(!turn && board->board[initial(m)]==bpawn && initial(m)%8 == 1)
	{
		...
		// Ganho de material:
		board->mat[blacks] += weights[*(reinterpret_cast<piece*>(&a))] - weights[bpawn];
	...
	else
	{
		...
		if(board->spc[turn].rkKing && ((captured == wrook && final(m) == 070) ||
		...
		if(board->spc[turn].rkQueen && ((captured == wrook && final(m) == 000) ||
...
bool isCheckmate(SearchBoard *board, Player turn)
{
	...
	// Verifica se ao rei está atacado:
	if(!isAttacked(board->board, (Player)!turn, board->kings[turn]))
		return false;

	moves = posMov(*board, turn);

	// Verifica se todos os lançes possíveis são inválidos:
    for(int i = 0; moves[i] != 0; i++)
    {
        B = *board;
...
bool isStalemate(SearchBoard *board, Player turn)
{
	...
	// Verifica se todos os lançes são inválidos:
    for(int i = 0; moves[i] != 0; i++)
    {
        B = *board;
        if(mkCmpMove(&B,turn,moves[i])!=invalid)
